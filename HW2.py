...
"""
Since all of the data except 'Age' is binary, it is only necessary to scale
the 'Age' column to between 0 and 1. Making 'Age' a one-hot vector is unnecessary,
as it creates many more columns = many more features, and makes things more complicated.
So, our one-hot vector is basically binary data and one feature scaled between 0 and 1.
The scaling of the 'Age' vector is done by subtracting the mean and then dividing by the
standard deviation (we assume the ages are normally distributed).
